...
BEGIN
	...
	IF TG_OP = 'UPDATE' THEN
    	objetAvant := '{'||OLD.moment||','||OLD.nombre||','||OLD.ageMoyen||','||OLD.checksum||'}';
   		objetApres := '{'||NEW.moment||','||NEW.nombre||','||NEW.ageMoyen||','||OLD.checksum||'}';
        operation := 'MODIFIER';
    END IF;
	...
	INSERT into journal(moment, operation, objet, description) VALUES(NOW(), operation, 'concepteur', description);
